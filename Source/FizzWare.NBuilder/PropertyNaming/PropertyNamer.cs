...
namespace FizzWare.NBuilder.PropertyNaming
{
	public abstract class PropertyNamer : IPropertyNamer
	{
		...
		public virtual void SetValuesOf<T>(T obj)
		{
			...
			foreach (PropertyInfo propertyInfo in type.GetProperties(FLAGS).Where(p => p.CanWrite))
			{
				SetMemberValue(propertyInfo, obj);
			}

			foreach (FieldInfo propertyInfo in type.GetFields().Where(f => !f.IsLiteral && type != typeof (DateTime)))
			{
				SetMemberValue(propertyInfo, obj);
			}
		}
			...
			if (memberInfo is PropertyInfo)
			{
				...
				catch (Exception)
				{
					...
					Trace.WriteLine(string.Format(
						"NBuilder warning: {0} threw an exception when attempting to read its current value", memberInfo.Name));
#endif
				}
			}
		...
		{
			object[] enumArray = EnumHelper.GetValues(enumType);
			return enumArray;
		}
	}
}